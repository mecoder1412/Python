...
def recommend_movies(genre=None, mood=None, rating=None, top_n=5):
    ...
    if rating:
         filtered_df=filtered_df[filtered_df['IMB_Rating'] >=rating]
    filtered_df=filtered_df.sample(frac=1).reset_index(drop=True)# Randomize order  
    recommendations=[]
...
# Show genres in a single line
 ...
 for idx, gen in enumerate(genre, 1):

   print(f"{Fore.CYAN}{idx}. {gen}")
